...
def next_column(previous_word, word, trellis)
  TAGS.map do |tag|
    prob_mass = TAGS.reduce(0) do |a, previous_tag|
      a + (TAG_PROBS[previous_tag][tag] * trellis[previous_tag][previous_word])
    end
    prob = WORD_PROBS[tag][word] * prob_mass
    {prob: prob, prev_prob_mass: prob_mass, tag: tag}
  end
end
